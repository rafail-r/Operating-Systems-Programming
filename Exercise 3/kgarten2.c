...
struct kgarten_struct { 
	 ...
	 */ 
	/* ... */ 
	int space;		/*space for new children*/ 
	sem_t door;	/*door of the class (semaphore)*/ 
	pthread_cond_t teacher_exit;	/*allow teacher to exit*/ 
	/* 
...
{ 
	...
	do{ 
		...
		pthread_mutex_lock(&thr->kg->mutex);			//lock 
		if (thr->kg->space) {   /*if there is space for other children (h porta htan anoixth alla metaxy mutex kai lock prolabe kai bghke 
daskalos h mphke paidi (akraia periptwsh) opote epanelegxos kai loop se periptwsh poy den katafere na mpei*/ 
        		++(thr->kg->vc);   //child goes in (children++) 
               thr->kg->space--;  //space-- 
               enter = 1;         //CHILD ENTERED 
             	if (thr->kg->space)//if there is space for other children 							open the door 
                                sem_post(&thr->kg->door); 
                } 
                pthread_mutex_unlock(&thr->kg->mutex);          //unlock 
        }while(!enter);	/*H logikh einai briskei porta anoixth paei na mpei (thn kleinei wait) kai telika blepei oti den exei xwro ara bgainei kai ksanaperimenei sthn porta*/ 
} 
...
{ 
	...
	pthread_mutex_lock(&thr->kg->mutex);	//lock 
	--(thr->kg->vc);					//child goes out (children--) 
	thr->kg->space++;		//space for one more children space++ 
	if(thr->kg->space > thr->kg->ratio) 
		pthread_cond_signal(&thr->kg->teacher_exit);	
//if there is enough space teacher is allowed to leave 
	else 
		sem_post(&thr->kg->door);			//else open the door 							(one child leaves - one can now get in) 
	pthread_mutex_unlock(&thr->kg->mutex);			//unlock 
} 
...
{ 
	...
	pthread_mutex_lock(&thr->kg->mutex);		//lock 
	++(thr->kg->vt);				//one more teacher (teachers++) 
	thr->kg->space += thr->kg->ratio;		//increase space for 										ratio more children 
	sem_post(&thr->kg->door);			//open the door 
	pthread_mutex_unlock(&thr->kg->mutex);		//unlock 
} 
...
{ 
	...
	pthread_mutex_lock(&thr->kg->mutex);				//lock 
	while(thr->kg->space < thr->kg->ratio)
		pthread_cond_wait(&thr->kg->teacher_exit, &thr->kg->mutex);	//wait until teacher can leave (CONDITION VARIABLE) 
	--(thr->kg->vt);		//one teacher leave (teacher--) 
	thr->kg->space -= thr->kg->ratio;	//space for more children is 								smaller 
	pthread_mutex_unlock(&thr->kg->mutex);				//unlock 
} 
...
{ 
	...
	pthread_cond_init(&kg->teacher_exit,NULL); 
	/* ... */ 
	/* 
	 * Create threads 
	 */ 
