...
void child_enter(struct thread_info_struct *thr)
{
	...
	while(!thr->kg->space)					//if no space child can not get in
        	pthread_cond_wait(&thr->kg->door,&thr->kg->mutex);	//child waits and when signal arrives wakes up and checks the condition (if there is space)
...
void child_exit(struct thread_info_struct *thr)
{
	...
	pthread_cond_signal(&thr->kg->teacher_exit);		//if there is a teacher that wants to leave send a signal to check if it's posible
	pthread_cond_signal(&thr->kg->door);			//or open the door (one child leaves - one can now get in)
	pthread_mutex_unlock(&thr->kg->mutex);			//unlock
}
...
void teacher_enter(struct thread_info_struct *thr)
{
	...
	pthread_cond_broadcast(&thr->kg->door);		//open the door for children
	pthread_cond_signal(&thr->kg->door);		//and tell to the teacher that waits to exit to check if it's posible
	pthread_mutex_unlock(&thr->kg->mutex);		//unlock
}
...
void teacher_exit(struct thread_info_struct *thr)
{
	...
	pthread_mutex_lock(&thr->kg->mutex);					//lock
	while(thr->kg->space < thr->kg->ratio)					//if space < ration teacher can not leave
		pthread_cond_wait(&thr->kg->teacher_exit, &thr->kg->mutex);	//wait until teacher can leave when signal arrives wake up and check the condition
	--(thr->kg->vt);							//one teacher leaves (teacher--)
...
int main(int argc, char *argv[])
{
	...
	pthread_cond_init(&kg->door,NULL);
